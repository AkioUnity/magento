...
class popup_model extends customizer{
  ...
  public function __construct($helper){
    $this->sessionId = isset($_COOKIE["PHPSESSID"]) ? $_COOKIE["PHPSESSID"] : session_id();
  ...
  public function getPopups() {                         
    ...
    $this->binds[':remoteAddr'] = $this->helper->getIp();
    ...
    if ($loggedIn and $this->helper->getPopupCookie('isSubscribed') == false){
  		$this->where[] = "(user_not_subscribed != 1)";
  	}
    ...
    /*
    foreach($this->binds as $param => $bind){
      $statement->bindParam($param, $bind, PDO::PARAM_STR);
    } 
    */ 
  ...
  public function addIpFilter() {  
      ...
      $this->binds[':userIp'] = $this->helper->getIp();
  }   
  ...
  public function addIfRefferalFilter() {
      ...
      if($refDomain!=$curDomain){
        $_SESSION['popupReferer'] = $url;
      }elseif(isset($_SESSION['popupReferer'])){
        $url = $_SESSION['popupReferer'];
      }
      
      $this->binds[':referalUrl'] = $url;
      $this->binds[':notReferalUrl'] = $url ? $url : 'something'; //otherwise notreferrals won't be null  
      
      $this->join[]  = "LEFT JOIN `".$this->getTable('magebird_popup_notreferral')."` AS `notreferrals` 
                          ON main_table.popup_id = notreferrals.popup_id AND :notReferalUrl LIKE notreferrals.not_referral";
  ...
  public function addNoResultsFilter(){  
      if($this->helper->getPopupCookie('resN')=='' || $this->helper->getPopupCookie('resN')===false){
        $this->where[] = "(search_results IS NULL OR search_results='')";
      }elseif($this->helper->getPopupCookie('resN')!==false && $this->helper->getPopupCookie('resN')!=''){        
        $results = $this->helper->getPopupCookie('resN');
        $this->where[] = "(search_results > ".$results." OR search_results IS NULL)"; 
        $this->helper->setPopupCookie('resN','');
      }
  ...
  public function addNVisitorFilter(){
      $nVisitor = intval($this->helper->getPopupCookie('nV'));
      $nVisitor = substr($nVisitor,-1);      
      $this->join[]  = "LEFT JOIN `".$this->getTable('magebird_popup_n_visitor')."` AS `nvisitor` 
                        ON main_table.popup_id = nvisitor.popup_id";
      $this->where[] = "(nvisitor.every_n_visitor IS NULL OR nvisitor.every_n_visitor = $nVisitor)"; 
  }  
  ...
  public function setNVisitor(){
    if(!$this->helper->getPopupCookie('nV')){         
      /*we can take any row from this table, it doesn't matter because all rows 
      are updated each time with 1 increase. Unless if user deleted popup, then row won't be increased.
      but it is not so important, when 10 new views will be generated data for every n visitor will be 
      correct again.
      */
      $table = $this->getTable('magebird_popup_stats');
      $sql = "SELECT MAX(visitors) FROM `$table`";
      $visitor = $this->pdo->query($sql);
      $nVisitor = $visitor->fetchColumn();  
      $this->helper->setPopupCookie('nV',$nVisitor,time()+(3600*48));
    }
  }
