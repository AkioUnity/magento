    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        {
            $itemWeight =  $item->getRowWeight() / $item->getQty();
            if($itemWeight < 150) {
                ...
                if($BoxDimension >= 0 && $BoxDimension <= 130) {
                    $combined_request->setPackageWeight($BoxDimension);
                    ...
                    $individual_rate += (99.0000 * $item->getQty());
                }
            } else {
                $individual_rate += (99.0000 * $item->getQty());
            }
            /*$combined_request->setPackageWeight($itemWeight);
            $temp_rate = $this->getRate($combined_request);
            $individual_rate += ($temp_rate['price'] * $item->getQty());*/
        }
        $individual_rate = ($individual_rate > 99) ? 99.0000 : $individual_rate;
        $rate['price'] = $individual_rate;
        /* Calculation complete here */
