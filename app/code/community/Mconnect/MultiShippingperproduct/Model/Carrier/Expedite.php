 ...
 */ 
class Mconnect_MultiShippingperproduct_Model_Carrier_Expedite
extends Mage_Shipping_Model_Carrier_Abstract
implements Mage_Shipping_Model_Carrier_Interface
{
    protected $_code = 'expediteshippingperproduct';
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request){
        ...
        if(Mage::getStoreConfig('carriers/expediteshippingperproduct/active')){
		...
		foreach($cartItems as $cartItem){
         ...
         foreach($items as $item) {
			...
			$Sipping_Rate_Attr = Mage::getModel('catalog/product')->load($item->getProductId())->getExpediteShippingRate();
					...
					$qty = $parentQty;
					  ...
					  $parent = Mage::getModel('catalog/product')->load($bundleProdId);
						//if(!in_array($bundleProdId,$addedProducts)){
							...
							$Sipping_Rate_Attr = $parent->getExpediteShippingRate();
			...
			if(($Sipping_Rate_Attr == '' || $Sipping_Rate_Attr == NULL) && Mage::getStoreConfig('carriers/expediteshippingperproduct/shipping_default_value_enable')){
                    $Sipping_Rate_Attr = Mage::getStoreConfig('carriers/expediteshippingperproduct/default_shipping_cost');
             } 
        ...
        if($this->getConfigData('allow_free_shipping') && $free_shipping_over_total < $grandTotal){
			...
			$method->setCarrier('expediteshippingperproduct');
			$method->setCarrierTitle($this->getConfigData('name'));			
			$method->setMethod('expediteshippingperproduct');    
    ...
    {
        return array('expediteshippingperproduct'=>$this->getConfigData('name'));
    }
