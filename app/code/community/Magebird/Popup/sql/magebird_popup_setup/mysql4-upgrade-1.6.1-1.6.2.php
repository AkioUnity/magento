...
$collection = Mage::getModel('core/config_data')
              ->getCollection()
              ->addFieldToFilter('path', array('like' => '%popup%'))
...
if(!isset($config['magebird_popup/general/trial_start'])){ 
  Mage::getModel('core/config')->saveConfig('magebird_popup/general/trial_start', time());
}

//Custom Grid add exception
if(Mage::helper('core')->isModuleEnabled('BL_CustomGrid')){
  $gridColl = Mage::getModel('core/config_data')
              ->getCollection()
              ->addFieldToFilter('path', array('like' => 'customgrid/global/exclusions_list'))
              ->getFirstItem()
              ->toArray(); 
  
  if(isset($gridColl['value'])){
    $excValue = unserialize($gridColl['value']);
  }else{
    $excValue = array();
  }            

  foreach ($excValue as $key => $exception) {
    if($exception['block_type']=='magebird_popup'){
      $popupExcAdded = true;
    }        
  }

  if(!$popupExcAdded){  
    array_push($excValue, array('block_type'=>'*magebird_popup*','rewriting_class_name'=>'*'));
    Mage::getModel('core/config')->saveConfig('customgrid/global/exclusions_list', serialize($excValue));    
  }   
      
}


if(isset($config['magebird_popup/licence/popup_essential'])){
  Mage::getModel('core/config')->saveConfig('magebird_popup/general/extension_key', $config['magebird_popup/licence/popup_essential']);
}
if (version_compare(Mage::getVersion(), '1.5', '<')){
  Mage::getModel('core/config')->saveConfig('magebird_popup/settings/requesttype', 3);
}else{
  Mage::getModel('core/config')->saveConfig('magebird_popup/settings/requesttype', 1);
}
Mage::getModel('core/config')->saveConfig('magebird_popup/settings/useajax', 1);
//popup renamed to magebird_popup due conflict with another popup module
if(isset($config['popup/general/extension_key']) && !isset($config['magebird_popup/general/licence_key'])){
  Mage::getModel('core/config')->saveConfig('magebird_popup/general/licence_key', $config['popup/general/extension_key']);
  //Mage::getModel('core/config')->saveConfig('magebird_popup/settings/useajax', $config['popup/settings/useajax']);
  //Mage::getModel('core/config')->saveConfig('magebird_popup/settings/requesttype', $config['popup/settings/requesttype']);
  $asyn = isset($config['popup/settings/asynchronous']) ? $config['popup/settings/asynchronous'] : 1;
  Mage::getModel('core/config')->saveConfig('magebird_popup/settings/asynchronous', $asyn);
  Mage::getModel('core/config')->saveConfig('magebird_popup/general/extension_key', $config['popup/general/extension_key']);
  Mage::getModel('core/config')->saveConfig('magebird_popup/general/trial_start', $config['popup/general/trial_start']);
  Mage::getModel('core/config')->saveConfig('magebird_popup/statistics/ajax_async', $config['popup/statistics/ajax_async']);
  Mage::getModel('core/config')->saveConfig('magebird_popup/statistics/ga_tracking', $config['popup/statistics/ga_tracking']);
  $mailchimp = isset($config['popup/services/mailchimp_option']) ? $config['popup/services/mailchimp_option'] : '';
  Mage::getModel('core/config')->saveConfig('magebird_popup/services/mailchimp_option', $mailchimp);
  $mailchimpKey = isset($config['popup/services/mailchimp_key']) ? $config['popup/services/mailchimp_key'] : '';
  Mage::getModel('core/config')->saveConfig('magebird_popup/services/mailchimp_key', $mailchimpKey);
  $doubleOptin = isset($config['popup/services/mailchimp_double_option']) ? $config['popup/services/mailchimp_double_option'] : '';
  Mage::getModel('core/config')->saveConfig('magebird_popup/services/mailchimp_double_option', $doubleOptin);  
}   
...
$mainTablecolumns = getMaintableColumns();
$templateTableColumns = getTemplateColumns(); 

$write = Mage::getSingleton('core/resource')->getConnection('core_write');
$mainTableExists=$write->query("SHOW TABLES LIKE '".$this->getTable('magebird_popup')."'");
...
if($tablePopupExists){
  $oldColumns = $write->query("SHOW COLUMNS FROM `".$this->getTable('magebird_popup')."`");
...
if($alterMainTable){
  ...
  $write->query("ALTER TABLE `".$this->getTable('magebird_popup')."` ".implode(" , ",$alter));
}

$checkTableExists=$write->query("SHOW TABLES LIKE '".$this->getTable('magebird_popup_content')."'");
if($checkTableExists->fetch()){
  $contentTableExists = true;
}else{
  $contentTableExists = false;
} 
...
$installer->run("
	...
	CREATE TABLE IF NOT EXISTS {$this->getTable('magebird_popup')} ( 
    ".implode(" , ",$mainTablecolumns).",                   
	  PRIMARY KEY (`popup_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;

  DROP TABLE IF EXISTS `{$this->getTable('magebird_popup_template')}`;
	CREATE TABLE `{$this->getTable('magebird_popup_template')}` ( 
    ".implode(" , ",$templateTableColumns).",                   
	  PRIMARY KEY (`template_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_store')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `store_id` smallint(6) unsigned NOT NULL,
    PRIMARY KEY (`popup_id`,`store_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;
    
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_page')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `page_id` smallint(6) unsigned NOT NULL,
    PRIMARY KEY (`popup_id`,`page_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8; 
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_content')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	...
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_product')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `product_id` int(11) unsigned NOT NULL,
    PRIMARY KEY (`popup_id`,`product_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_category')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `category_id` int(11) unsigned NOT NULL,
    PRIMARY KEY (`popup_id`,`category_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;  
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_country')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `country_id` varchar(10) NOT NULL,
    PRIMARY KEY (`popup_id`,`country_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;  
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_notcountry')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `country_id` varchar(10) NOT NULL,
    PRIMARY KEY (`popup_id`,`country_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;    
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_customer_group')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `customer_group_id` smallint(6) unsigned NOT NULL,
    PRIMARY KEY (`popup_id`,`customer_group_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;   
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_n_visitor')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `every_n_visitor` smallint(6) unsigned NOT NULL,
    PRIMARY KEY (`popup_id`,`every_n_visitor`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;      
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_day')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `day` smallint(6) unsigned NOT NULL,
    PRIMARY KEY (`popup_id`,`day`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;   
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_subscriber')}` (
	...
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_referral')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `referral` varchar(200) NOT NULL,
    PRIMARY KEY (`popup_id`,`referral`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;       
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_notreferral')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
  ...
  DELETE FROM {$this->getTable('magebird_popup_content')} WHERE is_template=1;  

	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_orders')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	  `order_id` int(11) NOT NULL,
    PRIMARY KEY (`popup_id`,`order_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8;      
  
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_popup_stats')}` (
	  `popup_id` smallint(6) unsigned NOT NULL,
	...
	CREATE TABLE IF NOT EXISTS `{$this->getTable('magebird_notifications')}` (
  ...
  INSERT IGNORE INTO {$this->getTable('magebird_popup_stats')} (popup_id) SELECT popup_id FROM {$this->getTable('magebird_popup')};          
");

$installer->endSetup();

//because old version doesn't have all fields
alterSubscriberTable($write,$this);

alterSalesruleTable($write,$this);

insertPopupTemplates($write,$this); 

renameBlocksTemplates($write,$this);
//old version doesn't have widget id inside widget, we need to insert it
updateWidgetIds($write,$this);


function renameBlocksTemplates($write,$obj){
  $query = "UPDATE ".$obj->getTable('magebird_popup')." SET popup_content = REPLACE(popup_content, :search, :replace)";
  $binds = array(
  'search' => "template=\"popup",
  'replace' => "template=\"magebird/popup"
  );
  ...
  $binds = array(
  'search' => "type=\"popup",
  'replace' => "type=\"magebird_popup"
  );
  $write->query($query, $binds); 
}           

function insertWidgetId($matches){
  return $matches[0].' widget_id="_'.substr( md5(rand()), 0, 4).'"';
}

function updateWidgetIds($write,$obj){
  $sql    = "SELECT popup_content,popup_id FROM ".$obj->getTable('magebird_popup');
  $rows   = $write->fetchAll($sql);
  $values = array();
  $binds = array();
  foreach($rows as $key => $row){
    $content = $row['popup_content'];
    $popupId = $row['popup_id'];
    if(strpos($content,"widget_id")===false){    
      $content = preg_replace_callback("/{{widget/","insertWidgetId",$content);                
      $values[] = "(".$popupId.",:content".$key.")";
      $binds[':content'.$key] = $content;
    }
  }
  
  if($values){
    $query = "INSERT INTO ".$obj->getTable('magebird_popup')." (popup_id,popup_content) 
              VALUES ".implode(",",$values)." ON DUPLICATE KEY UPDATE popup_content = VALUES(popup_content)";
    $write->query($query, $binds);  
  }  
}

function alterSubscriberTable($write,$obj){
  $columns = $write->query("SHOW COLUMNS FROM `".$obj->getTable('magebird_popup_subscriber')."` LIKE 'expiration_date'");
  if(!$columns->fetch()){
    $write->query("ALTER TABLE `".$obj->getTable('magebird_popup_subscriber')."` 
                   ADD `expiration_date` timestamp NULL DEFAULT NULL,
                   ADD `coupon_length` TINYINT NULL DEFAULT NULL,
                   ADD `coupon_prefix` VARCHAR( 20 ) NULL DEFAULT NULL
                   ");                 
  }
  $columns = $write->query("SHOW COLUMNS FROM `".$obj->getTable('magebird_popup_subscriber')."` LIKE 'user_ip'");
  if(!$columns->fetch()){
    $write->query("ALTER TABLE `".$obj->getTable('magebird_popup_subscriber')."`                    
...
function alterSalesruleTable($write,$obj){
  $columns = $write->query("SHOW COLUMNS FROM `".$obj->getTable('salesrule_coupon')."` LIKE 'is_popup'");
  if(!$columns->fetch()){
    $write->query("ALTER TABLE `".$obj->getTable('salesrule_coupon')."` 
                   ADD `is_popup` smallint(4) NULL DEFAULT NULL
                   ");                 
  }
  
  $columns = $write->query("SHOW COLUMNS FROM `".$obj->getTable('salesrule_coupon')."` LIKE 'user_ip'");
  if(!$columns->fetch()){
    $write->query("ALTER TABLE `".$obj->getTable('salesrule_coupon')."` 
                   ADD `user_ip` VARCHAR( 20 ) NULL DEFAULT NULL,
                   ADD `popup_cookie_id` VARCHAR( 20 ) NULL DEFAULT NULL
...
function insertPopupTemplates($write,$obj){
  $query = "
  INSERT IGNORE INTO `{$obj->getTable('magebird_popup_template')}`                                                                                                                                                                                                                                                                                                                                                                                          
  ...
  ('55','0','1','Popup Register with coupon (responsive)','','2','95','2','275','1',:description55,'','popup/preview/template55.jpg','','1','50','2','','6','0','10','0','1','#FFFFFF','1','1','0','1','100','3','1','0',:custom_css55,:custom_script55),
  ('56','0','1','Popup with social likes and coupon (responsive)','','2','95','2','320','1',:description56,'','popup/preview/template56.jpg','','1','50','2','#FFFFFF','3','5','20','1','2','#FFFFFF','1','1','0','1','100','3','1','0',:custom_css56,:custom_script56),
  ...
  ('59','2','1','Background image and transparent div (responsive)','Read instructions how to change image <a target=\"_blank\" href=\"http://www.magebird.com/magento-extensions/popup.html?tab=faq#designTips\">here</a>.','2','100','2','433','2',:description59,'','popup/preview/template59.jpg','','1','50','2','#FFFFFF','3','0','0','0','1','#FFFFFF','2','1','0','1','100','1','1','0',:custom_css59,:custom_script59),
  ...
  ('65','3','1','Newsletter coupon with countdown timer (responsive)','','2','100','2','850','1',:description65,'','popup/preview/template65.jpg','','1','50','2','','6','0','0','0','2','','1','1','100','1','60','5','1','0',:custom_css65,:custom_script65),
  ...
  'custom_script12' => "",
  'description13' => "<table border=\"0\">\r\n<tbody>\r\n<tr>\r\n<td>\r\n<h1><span style=\"color: #333333;\"><strong><span style=\"font-size: xx-large;\">Subscribe to our Discount Club</span></strong></span></h1>\r\n<p>To receive exclusive offers and our most popular news, please enter your email address and press subscribe.</p>\r\n<p>Don't worry, <strong>we hate spam too</strong> and we won't share your data with anybody!</p>\r\n</td>\r\n<td>\r\n<p><img src=\"{{media url=\"wysiwyg/popup/magebird.png\"}}\" alt=\"\" width=\"213\" height=\"192\" /></p>\r\n</td>\r\n</tr>\r\n</tbody>\r\n</table>\r\n<p>{{widget widget_id=\"_eft7\" type=\"magebird_popup/list\" template=\"magebird/popup/newsletter/2.phtml\" width=\"70\" width_unit=\"2\" button_color=\"#F9A300\" button_text_color=\"#FFFFFF\" on_success=\"1\" success_msg=\"Thank%20you%20for%20your%20subscription!\"}}</p>",
  'custom_css13' => ".dialogBody{\r\nbackground-image:linear-gradient(white, #e1e1e1);\r\n}",
  'custom_script13' => "",
  'description14' => "<table style=\"width: 100%; height: 190px;\" border=\"0\" cellspacing=\"0\" cellpadding=\"10\">\r\n<tbody>\r\n<tr>\r\n<td>\r\n<h1><span style=\"color: #333333;\"><strong><span style=\"font-size: xx-large;\">Subscribe to our Discount Club</span></strong></span></h1>\r\n<p>To receive exclusive offers and our most popular news, please enter your email address and press subscribe.</p>\r\n<p>Don't worry, <strong>we hate spam too</strong> and we won't share your data with anybody!</p>\r\n<p>&nbsp;</p>\r\n</td>\r\n<td>\r\n<p><img src=\"{{media url=\"wysiwyg/popup/magebird.png\"}}\" alt=\"\" width=\"213\" height=\"192\" /></p>\r\n</td>\r\n</tr>\r\n</tbody>\r\n</table>\r\n<div id=\"newsletterRibon\" style=\"background-color: #03a2b1;\"><img class=\"whiteArrow\" src=\"{{media url=\"wysiwyg/popup/whitearrow.png\"}}\" alt=\"\" />{{widget widget_id=\"_eft8\" type=\"magebird_popup/list\" template=\"magebird/popup/newsletter/7.phtml\" width=\"400\" width_unit=\"1\" button_color=\"#fcfcfc\" button_text_color=\"#00000\" on_success=\"1\" success_msg=\"Thank%20you%20for%20your%20subscription!\"}}</div>",
  ...
  'custom_css53' => "",
  'custom_script53' => "",
  'description54' => "<p style=\"margin-bottom: 0px; text-align: center;\"><span style=\"color: #f15b47; font-size: xx-large;\">Subscribe now</span></p>\r\n<p style=\"text-align: center;\"><span style=\"color: #333333;\"><strong><span style=\"font-size: xx-large;\">and get 5% off!</span></strong></span></p>\r\n<p style=\"text-align: center;\">{{widget widget_id=\"_rer4\" type=\"magebird_popup/list\" template=\"magebird/popup/newsletter/1.phtml\" width=\"100\" width_unit=\"2\" button_text=\"Get 5% off\" button_color=\"#f15b47\" button_text_color=\"#FFFFFF\" thanks_msg=\"<p style='font-size:18px'>To get discount copy & paste <strong>'Your coupon code goes here'</strong> to offer code box in your shopping cart.</p> <a href='#' class='dialogCloseCustom'>Back to the site &raquo;</a>\"}}</p>",
  'custom_css54' => "",
  'custom_script54' => "",
  'description55' => "<p><span style=\"font-size: large;\"><strong>Sign up and get 10% OFF!</strong></span></p>\r\n<p>{{widget widget_id=\"_rer5\" type=\"magebird_popup/register\" template=\"magebird/popup/register/2.phtml\" width=\"100\" width_unit=\"2\" button_color=\"#d83c3c\" button_text_color=\"#FFFFFF\" newsletter_option=\"2\" checkbox_text=\"Subscribe newsletter\" checkbox_checked=\"1\" coupon_option=\"1\" coupon_type=\"1\" coupon_code=\"YOUR COUPON GOES HERE\" send_coupon=\"2\" on_success=\"1\" success_msg=\"Your%20coupon%20is:%20%7B%7Bvar%20coupon_code%7D%7D.%20To%20get%20discount%20copy%20&%20paste%20it%20to%20offer%20code%20box%20in%20your%20shopping%20cart.\"}}</p>",
  'custom_css55' => "",
  'custom_script55' => "",
  'description56' => "<p style=\"text-align: center;\"><span style=\"color: #333333;\"><strong><span style=\"font-size: x-large;\">Like us and get </span><span style=\"color: #df0009; font-size: xx-large;\"><br />5%</span><span style=\"font-size: x-large;\"> off!</span></strong></span></p>\r\n<p style=\"text-align: center;\">{{widget widget_id=\"_rer6\" type=\"magebird_popup/follow\" template=\"magebird/popup/follow/1.phtml\" button_style=\"1\" coupon_type=\"1\" coupon_code=\"YOUR COUPON GOES HERE\" services=\"facebook,gplus,twitter,pinterest_follow\" success_msg=\"Thank%20you%20for%20your%20subscription!%20Your%20coupon%20is:%20%7B%7Bvar%20coupon_code%7D%7D.%20To%20get%20discount%20copy%20&%20paste%20it%20to%20offer%20code%20box%20in%20your%20shopping%20cart.\"}}</p>",
  'custom_css56' => ".dialogBody{\r\n    background: -webkit-radial-gradient(white, white, #e1e1e1); \r\n    background: -o-radial-gradient(white, white, #e1e1e1);\r\n    background: -moz-radial-gradient(white, white, #e1e1e1); \r\n    background: radial-gradient(white, white, #e1e1e1);\r\n}",
  'custom_script56' => "",
  'description57' => "<h1 style=\"background-color: #4c4b4b; color: white; line-height: 42px;\"><span style=\"color: #ffffff; font-size: 30px;\">10% OFF. Join discount club!<br /></span></h1>\r\n<p>Join our discount club and get <strong>10% OFF</strong> now. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin commodo, nisi mollis mattis laoreet</p>\r\n<p>{{widget widget_id=\"_rer7\" type=\"magebird_popup/list\" template=\"magebird/popup/newsletter/2.phtml\" width=\"100\" width_unit=\"2\" button_color=\"#4c4b4b\" button_text_color=\"#FFFFFF\" apply_coupon=\"1\" on_success=\"1\" success_msg=\"Thank%20you%20for%20your%20subscription!\"}}</p>\r\n<p><span style=\"font-size: x-small;\">*We will never share your information</span></p>",
  ...
  'description73' => "<table style=\"background-image: url('{{media url=\"wysiwyg/popup/board.png\"}}'); width: 100%; height: 510px;\" border=\"0\">\r\n<tbody>\r\n<tr>\r\n<td style=\"padding: 30px 115px 0 115px; vertical-align: top;\">\r\n<div style=\"margin: 0 auto;\">\r\n<p class=\"fontBig\" style=\"text-align: left; font-size: 70px; line-height: 1; margin: 0;\">EXCLUSIVE</p>\r\n<p class=\"fontBig\" style=\"text-align: left; font-size: 70px; line-height: 1; font-weight: bold; margin: 0; color: #ff1176;\">SAVINGS!</p>\r\n<p class=\"popupSmall\" style=\"text-align: left; font-size: 24px; line-height: 1; margin: 20px 0; font-style: italic;\">Enter your e-mail and hear<br />about exclusive savings and news!</p>\r\n<div style=\"text-align: left;\">{{widget type=\"magebird_popup/list\" widget_id=\"_vd49\" template=\"magebird/popup/newsletter/10.phtml\" width=\"100\" width_unit=\"2\" button_text=\"Sign me up!\" button_color=\"#ff1176\" button_text_color=\"#ffffff\" apply_coupon=\"1\" on_success=\"1\" success_msg=\"%3Cp%20style=%22padding:30px;%20font-size:20px;%20text-align:center;%20background-color:white;%20border:1px%20solid%20silver%22%3EThank%20you%20for%20your%20subscription!%3C/p%3E\"}}</div>\r\n<p class=\"dialogCloseCustom popupSmall\" style=\"text-decoration: underline; text-align: left; font-size: 18px; font-style: italic; margin-top: 10px;\">No thanks, I will pay full price</p>\r\n</div>\r\n</td>\r\n</tr>\r\n</tbody>\r\n</table>",
...
function newLicence(){
  $resp = null;
  $data=http_build_query(array("extension" =>"popup","domain"=>$_SERVER['HTTP_HOST'],"affId"=>0));
...
function getMaintableColumns(){
  return array(
    ...
    "cart_qty_min"=>"`cart_qty_min` smallint(6) NOT NULL default '0'",
    "cart_qty_max"=>"`cart_qty_max` smallint(6) NOT NULL default '0'",
    ...
    "search_results"=>"`search_results` int(11) NULL default NULL",
